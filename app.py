...
/* --- Updated Header Styling: White with a Faint Cyan Glow --- */
h1, h2, h3, .stHeader, [data-testid="stHeader"] h1, [data-testid="stSidebar"] h1, [data-testid="stSidebar"] h2, h4 {{
    ...
    text-shadow: 0 0 12px rgba(0, 255, 255, 0.45) !important; /* Faint Cyan Glow */
...
/* --- Glassmorphism shifted to Navy Blue --- */
.circuit-code {{
    background-color: rgba(10, 20, 50, 0.6) !important; /* Navy Blue Base */
    backdrop-filter: blur(15px) saturate(180%) !important;
    color: #FFFFFF !important; /* Clean white code */
    text-shadow: 0 0 10px rgba(0, 255, 255, 0.3) !important; /* Subtler cyan glow on code */
    padding: 20px !important;
    border-radius: 10px !important;
    border: 1px solid rgba(0, 255, 255, 0.2) !important; /* Faint cyan border */
...
[data-testid="stAppViewContainer"] {{
    background: linear-gradient(rgba(10, 15, 30, 0.7), rgba(5, 10, 20, 0.9)), {bg_image_css};
...
.vignette {{
    position: fixed; top: 0; left: 0; width: 100vw; height: 100vh;
    background: radial-gradient(circle, transparent 45%, rgba(0,0,15,0.98) 120%);
    pointer-events: none; z-index: 10;
}}
.heat-shimmer {{
    position: fixed; top: 40%; left: 50%; width: 600px; height: 800px;
    transform: translate(-50%, -50%);
    background: radial-gradient(ellipse, rgba(0, 255, 255, 0.15) 0%, rgba(0, 200, 255, 0.05) 40%, transparent 70%); /* Shimmer shifted to Cyan */
...
@keyframes blink-caret {{ from, to {{ border-color: transparent }} 50% {{ border-color: #00FFFF; }} }} /* Cyan caret */
@keyframes pulse_shimmer {{
    0% {{ opacity: 0.4; border-color: rgba(0, 255, 255, 0.1); }}
    50% {{ opacity: 0.8; border-color: rgba(0, 255, 255, 0.4); box-shadow: 0 0 10px rgba(0,255,255,0.2) }}
    100% {{ opacity: 0.4; border-color: rgba(0, 255, 255, 0.1); }}
}}
.shimmer-box {{
    animation: pulse_shimmer 3s infinite ease-in-out !important;
    border: 1px dashed rgba(0, 255, 255, 0.5) !important;
    background-color: rgba(10, 20, 50, 0.4) !important;
}}

/* --- HUD Box Glassmorphism (Navy + Cyan Accent) --- */
.hud-box {{
    animation: breathe 6s infinite ease-in-out;
    background: linear-gradient(135deg, rgba(20,40,80,0.6), rgba(10,20,40,0.8)); /* Deep Navy */
    backdrop-filter: blur(10px); -webkit-backdrop-filter: blur(10px);
    border: 1px solid rgba(0, 255, 255, 0.15); border-radius: 16px;
    padding: 1.5rem; transition: all 0.2s ease-in-out; box-shadow: 0 4px 15px rgba(0, 0, 0, 0.4);
}}
.hud-box:hover {{
    transform: translateY(-2px); border: 1px solid rgba(0, 255, 255, 0.4);
    background: linear-gradient(135deg, rgba(30,50,100,0.7), rgba(15,30,60,0.9));
    box-shadow: 0 8px 25px rgba(0, 255, 255, 0.15);
}}

/* --- Button Styling: Cyan accents --- */
div.stButton > button {{
    ...
    border: 1px solid rgba(0, 255, 255, 0.4) !important; border-radius: 8px !important;
}}
div.stButton > button > div > p {{
    background: none;
    -webkit-text-fill-color: #FFFFFF !important; /* White button text */
    text-shadow: 0 0 10px rgba(0, 255, 255, 0.5) !important; /* Cyan glow */
    font-weight: 800 !important;
}}
div.stButton > button:hover {{
    transform: scale(1.05) !important;
    background: linear-gradient(135deg, rgba(0, 255, 255, 0.2), rgba(0, 255, 255, 0.3)) !important;
    box-shadow: 0 0 20px rgba(0, 255, 255, 0.5) !important; border: 1px solid #00FFFF !important;
}}
...
/* --- Briefing Text Glassmorphism (Navy + Cyan Accent) --- */
.briefing-text {{
    background: linear-gradient(135deg, rgba(10,20,50,0.85), rgba(5,10,30,0.6)); /* Deep Navy */
    padding: 35px; border-radius: 15px; border-left: 5px solid #00FFFF; /* Cyan Left Border */
...
.briefing-text:hover {{ box-shadow: 0 10px 40px rgba(0,255,255,0.3); border-left: 5px solid #00FFFF; }} /* Glowing hover effect */

/* --- Sidebar Glassmorphism (Navy + Cyan Accent) --- */
[data-testid="stSidebar"] {{
    background: rgba(10, 20, 50, 0.4) !important; /* Navy base */
    backdrop-filter: blur(30px) saturate(150%) !important;
    -webkit-backdrop-filter: blur(30px) saturate(150%) !important;
    border-right: 1px solid rgba(0, 255, 255, 0.2) !important; box-shadow: 5px 0 30px rgba(0, 0, 0, 0.6) !important;
}}
...
if st.session_state.tutorial_step < 5:
    ...
    with st.container():
        ...
        if st.session_state.tutorial_step == 0:
            st.markdown(f"""
            <div style="font-family:'Orbitron', sans-serif; font-size: 2.2rem; color:#FFF; text-shadow: 0 0 20px rgba(0,255,255,0.8); white-space: nowrap; overflow: hidden; border-right: 0.15em solid #00FFFF; display: inline-block; animation: typing 2.5s steps(35, end) forwards, blink-caret 0.75s step-end infinite;">
                INITIALIZING MISSION CRITICAL SYSTEMS
            </div>
            """, unsafe_allow_html=True)
            # Swapped to <div> and bumped size to 1.8rem!
            st.markdown('<div style="margin-top:15px; font-family: \'JetBrains Mono\', monospace; color: #E0E0E0; font-size: 20px; line-height: 1.6;">Welcome, Lead Researcher. QuantumLens v1 is now online.</div>', unsafe_allow_html=True)
            ...
            st.markdown('<div style="font-family: \'JetBrains Mono\', monospace; color: #E0E0E0; font-size: 20px; line-height: 1.6;">In classical logic, a bit is binary. In this lab, a Qubit exists as a linear combination of |0⟩ and |1⟩.</div>', unsafe_allow_html=True)
            st.markdown('<div style="font-family: \'JetBrains Mono\', monospace; color: #00FFCC !important; font-size: 18px; margin-top: 10px;">TELEMETRY: Hadamard (H) Gate active.</div>', unsafe_allow_html=True)
            ...
            st.markdown('<div style="font-family: \'JetBrains Mono\', monospace; color: #E0E0E0; font-size: 20px; line-height: 1.6;">By engaging the <b style="color:#00FFFF;">CX-Gate</b>, we link the fates of Q_0 and Q_1.</div>', unsafe_allow_html=True)    
            st.markdown('<div style="font-family: \'JetBrains Mono\', monospace; color: #ff4b4b !important; font-size: 18px; margin-top: 10px;">WARNING: Measuring one entangled qubit affects the other.</div>', unsafe_allow_html=True)
            ...
            st.markdown('<div style="font-family: \'JetBrains Mono\', monospace; color: #E0E0E0; font-size: 20px; line-height: 1.6;">The Observer Effect is final. When you hit MEASURE, the superposition vanishes.</div>', unsafe_allow_html=True)
